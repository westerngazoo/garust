...
pub trait Algebra: Copy + Debug {
    ...
    /// Precomputed **sparse** wedge (outer-product) table: only the `3^N`
    /// disjoint blade pairs that survive the wedge, in CSR layout (see
    /// [`WedgeTable`](crate::signature::WedgeTable) for why sparse beats a
    /// dense `CAYLEY`-style table here).
    ///
    /// The wedge is metric-independent, so this table is the same for every
    /// signature of a given dimension; it is generated by
    /// [`define_algebra!`](crate::define_algebra) and the `Algebra` derive
    /// from [`wedge_rows`](crate::signature::wedge_rows) and
    /// [`wedge_pairs`](crate::signature::wedge_pairs).
    const WEDGE: crate::signature::WedgeTable;
}
    ...
    ($(#[$meta:meta])* $vis:vis $name:ident = Cl($p:literal, $q:literal, $r:literal) $(;)?) => {
        ...
        impl $crate::algebra::Algebra for $name {
            ...
            const CAYLEY: &'static [$crate::signature::CayleyEntry] =
                ...
                >(1usize << ($p + $q + $r), $p, $q);
            const WEDGE: $crate::signature::WedgeTable = $crate::signature::WedgeTable {
                rows: &$crate::signature::wedge_rows::<
                    { (1usize << ($p + $q + $r)) + 1 },
                >(1usize << ($p + $q + $r)),
                pairs: &$crate::signature::wedge_pairs::<
                    { $crate::signature::pow3($p + $q + $r) },
                >(1usize << ($p + $q + $r)),
            };
        }
    };
